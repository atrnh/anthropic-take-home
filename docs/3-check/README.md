...
The default run creates `docs/3-check/output.local/` with:
...
```sh
uv run docs/3-check/run.py --top-k 10 --output-dir /tmp/docs-review
uv run docs/3-check/run.py --corpus /path/to/snapshot --output-dir /tmp/other-review
```
...
```sh
uv run docs/3-check/run.py --advice /path/to/advice.json
```
...
```sh
uv run docs/3-check/test_workflow.py
uv run python docs/3-check/docslint/test_archive.py
