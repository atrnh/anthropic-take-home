...
Keep the original defaults and use v3 explicitly for the next calibration. Reject
v2. Resolve the FAQ policy with an editor and obtain fresh positive examples
...
The summary-versus-FAQ policy remains open for human adjudication. The revised
rubric explicitly defers short same-page feature-summary/FAQ repetitions with
...
All three fresh judges were requested as GPT-5.6-Terra with high effort and were
...
uv run docslint/judge_pilot.py prepare --sample-offset 2 \
  --output-dir /tmp/claude-judge-v2-control
cmp docs/3-check/judge-v2/judge-packet.json /tmp/claude-judge-v2-control/judge-packet.json
cmp docs/3-check/judge-v2/control-manifest.json /tmp/claude-judge-v2-control/judge-manifest.json
uv run docslint/judge_pilot.py prepare --sample-offset 2 \
  --prompt docs/3-check/judge-v2/prompt.md --output-dir /tmp/claude-judge-v2
cmp docs/3-check/judge-v2/judge-packet.json /tmp/claude-judge-v2/judge-packet.json
cmp docs/3-check/judge-v2/judge-manifest.json /tmp/claude-judge-v2/judge-manifest.json
uv run docslint/judge_pilot.py evaluate \
  --packet docs/3-check/judge-v2/judge-packet.json \
  --manifest docs/3-check/judge-v2/control-manifest.json \
  --judgments docs/3-check/judge-v2/control-decisions.json > /tmp/claude-control-metrics.json
cmp docs/3-check/judge-v2/control-metrics.json /tmp/claude-control-metrics.json
uv run docslint/judge_pilot.py evaluate \
  --prompt docs/3-check/judge-v2/prompt.md \
  --packet docs/3-check/judge-v2/judge-packet.json \
  --manifest docs/3-check/judge-v2/judge-manifest.json \
  --judgments docs/3-check/judge-v2/judge-decisions.json > /tmp/claude-v2-metrics.json
cmp docs/3-check/judge-v2/judge-metrics.json /tmp/claude-v2-metrics.json
uv run docslint/judge_pilot.py prepare --sample-offset 2 \
  --prompt docs/3-check/judge-v2/prompt-v3.md --output-dir /tmp/claude-judge-v3
cmp docs/3-check/judge-v2/judge-packet.json /tmp/claude-judge-v3/judge-packet.json
cmp docs/3-check/judge-v2/v3-manifest.json /tmp/claude-judge-v3/judge-manifest.json
uv run docslint/judge_pilot.py evaluate \
  --prompt docs/3-check/judge-v2/prompt-v3.md \
  --packet docs/3-check/judge-v2/judge-packet.json \
  --manifest docs/3-check/judge-v2/v3-manifest.json \
  --judgments docs/3-check/judge-v2/v3-decisions.json > /tmp/claude-v3-metrics.json
cmp docs/3-check/judge-v2/v3-metrics.json /tmp/claude-v3-metrics.json
```
