# Editorial judge prompt, version 3
...
that could be consolidated. An unresolved canonical home alone is not a reason
to reject a clear duplication finding.

The policy for a short fact repeated in a same-page feature summary and direct
FAQ answer is awaiting human adjudication. For that pattern, set `needs_review`
to `true` and explain the direct-answer versus single-explanation tradeoff. This
is a temporary deferral, not a correct negative classification.
